...
namespace RealtimeCSG
{
	...
	internal static class NativeMethodBindings
	{
		...
		public static void RegisterUnityMethods()
		{
			...
			{
				Debug.Log(message, (uniqueObjectID != 0) ? EntityIdRegistry.IdToObject(uniqueObjectID) : null);
			};
			unityMethods.DebugLogError      = delegate (string message, int uniqueObjectID) 
			{
				Debug.LogError(message, (uniqueObjectID != 0) ? EntityIdRegistry.IdToObject(uniqueObjectID) : null);
			};
			unityMethods.DebugLogWarning    = delegate (string message, int uniqueObjectID) 
			{
				Debug.LogWarning(message, (uniqueObjectID != 0) ? EntityIdRegistry.IdToObject(uniqueObjectID) : null);
			};
			unityMethods.NameForUserID		= delegate (int uniqueObjectID)
			{
				var obj = (uniqueObjectID != 0) ? EntityIdRegistry.IdToObject(uniqueObjectID) : null;
				if (obj == null)
					return "<unknown>";
				else
					return obj.name;
												  ...
												  CSGBrush[]				ignoreBrushes = null)
		{
			...
			{
				var obj					= EntityIdRegistry.IdToObject(__outputIntersections[i].brushUserID);
											  ...
											  HashSet<GameObject>	gameObjects)
		{
			...
			{
				var obj			= EntityIdRegistry.IdToObject(ids[i]);
